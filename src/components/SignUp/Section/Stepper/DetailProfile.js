...
const DetailProfile = ({ onChange, onCheck, Preview, checked }) => {
  ...
  return (
    <ModalContainer>
      ...
      <FormBox paddingTop="24px">
        ...
        <LabelDescription>
          이미지 업로드를 하지 않을 시 기본 프로필로 지정되며 가입 후
          마이페이지에서 변경 가능합니다.
        </LabelDescription>
        <ImgContainer>
          <RadioBox>
        ...
        <EmailBox>
          ...
          <Description>
            <div>
              <Icon>※</Icon> 가입 시 입력한 이메일 메일함을 확인해주세요!
            </div>
            <div>
              <strong>
                <Icon>●</Icon> 이메일 도용방지, 댓글 쓰기 권한 활성화
              </strong>
              를 위한 인증 메일이 발송됩니다.
            </div>
            <div>
              <Icon>●</Icon> 인증 메일은 30분간 유효하며, 인증 메일이 만료된
              경우 정보 수정 페이지에서 인증메일을 다시 요청할 수 있습니다.
            </div>
            <div>
              <Icon>●</Icon> 이메일 주소가 사용되지 않거나 없는 주소인 경우 정보
              수정 페이지에서 다른 이메일 주소로 인증 메일을 요청할 수 있습니다.
            </div>
          </Description>
        </EmailBox>
      </FormBox>
